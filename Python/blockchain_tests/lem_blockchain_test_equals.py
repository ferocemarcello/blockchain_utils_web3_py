...
def test_equals_offers_bids():
    open_offs_db, open_bds_db = [tuple(x) for x in _convert_qualities_to_int(db_obj, open_offers_db, config['lem']['types_quality']).values.tolist()], [tuple(x) for x in _convert_qualities_to_int(db_obj, open_bids_db, config['lem']['types_quality']).values.tolist()]
    offs_db_archive, bds_db_archive = [tuple(x) for x in _convert_qualities_to_int(db_obj, offers_db_archive, config['lem']['types_quality']).values.tolist()], [tuple(x) for x in _convert_qualities_to_int(db_obj, bids_db_archive, config['lem']['types_quality']).values.tolist()]
