...
def clearTempData():
    try:
        ...
        limit_to_remove = 500
        while len(getOffers_or_Bids(isOffer=True, temp= True)) > 0 or len(getOffers_or_Bids(isOffer=False, temp=True)) > 0 or len(functions.getTempMarketResults().call()) > 0 or len(functions.getMarketResultsTotal().call()) > 0:
            try:
...
def clearPermanentData():
    try:
        ...
        # exceptions happens when the cost of deletion is too big. then we have to delete chunk by chunk
        limit_to_remove = 500
        while len(getOffers_or_Bids(isOffer=True, temp=False)) > 0 or len(getOffers_or_Bids(isOffer=False, temp=False)) > 0 or len(functions.get_user_infos().call()) or len(functions.get_id_meters().call()):
            try:
